...
class HocVienHoanThanhResource extends Resource
{
    ...
    protected static ?bool $khoaHocHasTrainingTypeColumn = null;
    ...
    public static function applyFilterConstraints(Builder $query, array $data): Builder
    {
        ...
        if (! empty($trainingTypes)) {
            $query->whereHas('khoaHoc', function (Builder $courseQuery) use ($trainingTypes) {
                self::applyTrainingTypeConstraint($courseQuery, $trainingTypes);
            });
        }
    ...
    protected static function applyTrainingTypeConstraint(Builder $courseQuery, array $trainingTypes): void
    {
        $types = collect($trainingTypes)
        ...
        $courseQuery->where(function (Builder $builder) use ($types) {
            $builder->whereHas('chuongTrinh', fn (Builder $q) => $q->whereIn('loai_hinh_dao_tao', $types));

            if (self::hasKhoaHocTrainingTypeColumn()) {
                $builder->orWhereIn('loai_hinh_dao_tao', $types);
            }
        });
    }

    protected static function hasKhoaHocTrainingTypeColumn(): bool
    {
        if (self::$khoaHocHasTrainingTypeColumn !== null) {
            return self::$khoaHocHasTrainingTypeColumn;
        }

        return self::$khoaHocHasTrainingTypeColumn = Schema::hasColumn('khoa_hocs', 'loai_hinh_dao_tao');
    }
