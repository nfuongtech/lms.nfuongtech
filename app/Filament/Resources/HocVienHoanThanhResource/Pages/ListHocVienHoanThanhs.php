...
class ListHocVienHoanThanhs extends ListRecords
{
    ...
    public function exportExcel()
    {
        ...
        $columnKeys = $this->getVisibleExportColumnKeys();

        if (empty($columnKeys)) {
            Notification::make()->title('Vui lòng hiển thị ít nhất một cột trước khi xuất')->warning()->send();
            return null;
        }

        $filters = $this->resolveFilterState();

        return HocVienHoanThanhResource::export(
            $records,
            $columnKeys,
            $this->summaryRows,
            $filters,
    ...
    protected function getVisibleExportColumnKeys(): array
    {
        $definitions = HocVienHoanThanhResource::getExportColumnDefinitions();
        $toggleState = data_get($this->tableColumnToggleFormData ?? [], 'columns', []);

        return collect(array_keys($definitions))
            ->filter(function (string $column) use ($toggleState) {
                if (! is_array($toggleState) || ! array_key_exists($column, $toggleState)) {
                    return true;
                }

                return (bool) $toggleState[$column];
            })
            ->values()
            ->all();
    }
