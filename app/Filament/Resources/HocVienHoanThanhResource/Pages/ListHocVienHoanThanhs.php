...
class ListHocVienHoanThanhs extends ListRecords
{
        ...
        return [
            Actions\Action::make('export_excel')
                ...
                ->action(function () {
                    ...
                    if (empty($columnKeys)) {
                        Notification::make()->title('Vui lòng hiển thị ít nhất một cột trước khi xuất Excel')->warning()->send();
                        return null;
                    }
    ...
    protected function getVisibleExportColumnKeys(): array
    {
        ...
        return collect($this->getTableColumns())
            ...
            ->map(function ($column) {
                return method_exists($column, 'getName') ? $column->getName() : null;
            })
            ->filter(fn ($name) => $name !== null && array_key_exists($name, $definitions))
            ->values()
            ->all();
    }
